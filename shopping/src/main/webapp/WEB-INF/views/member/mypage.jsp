...
<script type="text/javascript">
	function fileImageSet(){
		location.href ="/member/fileImageSet/?fname=${dto.fname}&id=${dto.id}";
	}
</script>
</head>
<body>
		...
		<div class="row">
			<div class="col-sm-3">
				...
				<img class="img-rounded" src="/member/storage/${dto.fname}"
					style="width: 280px">
				<!-- 사진 수정 -->
				<div class="imageSet">${dto.fname}
					<button class="btn btn-default" onclick="javascript:fileImageSet()">사진 수정</button>
				</div>
								...
								<li class="list-group-item">결제금액 : ${order.total} 원, 주문상태 :
									${order.ostate} <c:forEach var="detail" items="${order.list}">
										<li class="list-group-item">${detail.pname},
											${detail.quantity}개 <a
											href="review/create/${detail.contentsno}"><span
												class="badge">Review</span></a>
										</li>
									</c:forEach>
							</c:forEach>
						</c:otherwise>
