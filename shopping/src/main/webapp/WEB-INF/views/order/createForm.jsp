...
function order(tot){
	let payment = document.querySelector('.btn-check:checked').value //체크박스에선 이렇게사용할수없다 , radio 버튼은 하나만체크되니까 checked 된것만 선택할수있다.
	let req = document.querySelector('#req');
	let reqtext = req.options[req.selectedIndex].text;     //전체option 을 index 로 접근할수있다.  selectedIndex = select 에서 선택된 index 
	let f = document.querySelector('#oform');
	//form 을찾아서 input name 과 일치하는 태그에 value 에 값을 넣어준다.
	f.tot.value = tot;
	f.payment.value = payment;
	f.reqtext.value =reqtext;
	//form 에 action 을 여기서도 설정해줄수있다.
...
<div class="container">
  ...
  <c:forEach var="dto" items="${list}" varStatus="a"> <!-- forEach 기능중 varStatus 가 index 를 뽑아준다. -->
  <c:if test="${str == 'cart'}" >
  <c:set var="qty" value='${(quantity).split(",")[a.index] }'/>
  </c:if>
  <c:set var="tot" value="${  tot + (dto.price * qty)  }"/>
  
  <figure>
  <img class="img-rounded" src="/contents/storage/${dto.filename}" style="width:100px;height:150px" >
  ...
  <ul class="list-group">
	...
	<input type="radio" class="btn-check" id="btn1" name="payment" value="신용카드" checked>
	<label class="btn btn-primary" for="btn1">신용카드</label></input> 
		
	<input type="radio" class="btn-check" id="btn2" name="payment">
	<label class="btn btn-primary" for="btn2">pay 카드</label></input> 
	
	<input type="radio" class="btn-check" id="btn3" name="payment">
	<label class="btn btn-primary" for="btn3">pay 머니</label></input>
...
<form id="oform" method="post">
	<input type="hidden" name="cno" value="${cno}" /> <!-- 장바구니를 통해서 들어온값 -->
	<input type="hidden" name="contentsno" value="${contentsno}" /> <!-- 장바구니없이 주문들어온값 -->
